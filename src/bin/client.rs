...
//reads from stdin and then sends the data 
//to the tcp stream
async fn send_commands(mut to_server: net::TcpStream) -> RusqlResult<()> {
    println!("Commands:\n\
              join GROUP\n\
              post GROUP MESSAGE...\n\
    ...
    //wait why didn't we need to call lock on stdin?
...
use async_rusql::FromServer;

async fn handle_replies(from_server: net::TcpStream) -> RusqlResult<()> {
